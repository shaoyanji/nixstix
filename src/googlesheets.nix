...
in
  ...
  */
  ''
    mkdir -p $out
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>"${title}"</title><link rel="stylesheet" href="${style}" type="text/css"><style type="text/css"></style></head><body>' >> ${index}
    echo '
     <div id="pageui" x-data="'"{ recipes: '''}"'" x-init="recipes = await fetchrecipe().then((data) => data )">

         <ul>
         <template x-for="recipe in recipes">
             <details>
                 <summary x-text="recipe[0]"></summary>
                 <h3>Ingredients</h3>
